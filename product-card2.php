...
?>
    ...
    for ($i = 0; $i < $getProduct->num_rows; $i++) {
        ...
        if (isset($_GET['size'])) {
            ...
            if (!empty($_GET['size'])) {
                $checkSize = Database::search("SELECT * FROM `product_size_has_product` WHERE `product_size_has_product`.`product_id` = '" . $row["id"] . "' AND `product_size_has_product`.`product_size_size_id` = '" . $_GET['size'] . "'");
